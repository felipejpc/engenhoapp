...
class ContentfulPostsSynchronization
  ...
  def self.initial_sync_posts
    ...
    Sync.new(content_type: "blogPost", sync_type: "initial entries",
    ...
    Sync.new(content_type: "blogPost", sync_type: "initial deletion",
             sync_token: sync.next_sync_url.nil? ? nil : sync.next_sync_url.split("sync_token=")[1]).save
  end
  ...
  def self.delta_sync_posts
    ...
    sync.each_item do |item|
      if new_entry? item.id
        ...
        post&.update(title: item.title, slug: item.slug, description: item.description)
      end
    end
    Sync.new(content_type: "blogPost", sync_type: "delta entries",
             sync_token: sync.next_sync_url.nil? ? nil : sync.next_sync_url.split("sync_token=")[1]).save
    ...
    unless sync.nil?
      ...
      Sync.new(content_type: "blogPost", sync_type: "delta deletion",
               sync_token: sync.next_sync_url.nil? ? nil : sync.next_sync_url.split("sync_token=")[1]).save
    end
  end
